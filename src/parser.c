...
layer parse_crnn(list *options, size_params params) {
    ...
    layer l = make_crnn_layer(params.batch, params.w, params.h, params.c, hidden_filters, output_filters,
                              params.time_steps, activation, batch_normalize);
...
//    printf("biases: %d\n", l.n);
    if (l.batch_normalize && (!l.dontloadscales)) {
        ...
        fread(l.rolling_variance, sizeof(float), l.n, fp);
//        weight_count += 3;
//        printf("scales,roolling_mean,rooling_variance   3 * %d = %d \n", l.n, 3 * l.n);
        if (0) {
            int i;
            for (i = 0; i < l.n; ++i) {
                printf("%g, ", l.rolling_mean[i]);//%f or %e
            }
            printf("\n");
            for (i = 0; i < l.n; ++i) {
...
// major = 0, minor = 1, revision = 0, transpose = 0
//    weight_count += 4;
//    printf("Load weight...major, minor, revision, seen\t%d\n", 4);
    int i;
    for (i = 0; i < net->n && i < cutoff; ++i) {
        layer l = net->layers[i];
//        printf("type: %d\n", l.type);
